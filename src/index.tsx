...
function activate (
    ...
    menu: IMainMenu,
    restorer: ILayoutRestorer | null): void {
  ...
  app.commands.addCommand(command, {
    ...
    execute: async () => {
      if (widget == undefined || !widget.isAttached) {
        ...
        const shortCutsFromRegistry = await settingRegistry.load(pluginLocation);
        const shortCutsList = Object.keys(shortCutsFromRegistry.composite);
        widget = createWidget(shortCutsList, settingRegistry, app, pluginLocation, label);
      }
...
function createWidget(
    ...
    label: string) : MainAreaWidget<ShortcutWidget> {
  const widget: ShortcutWidget = new ShortcutWidget(-1, -1, commandlist, 
    settingRegistry, app.commands, pluginLocation, app);
